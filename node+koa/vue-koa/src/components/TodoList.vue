...
export default {
  ...
  methods: {
      ...
      this.$http.get('/api/todolist/' + this.id) // 向后端发送获取todolist的请求
        .then((res) => {
          if(res.status == 200){
            ...
            this.$message.error('获取列表失败1！')
          }
        }, (err) => {
          this.$message.error('获取列表失败2！')
          console.log(err)
        })
    },
    addTodos () {
    ...
    update(index) {
      this.$http.get('/api/todolist/'+ this.id + '/' + this.list[index].id + '/' + this.list[index].status)
        .then((res) => {
          if(res.status == 200){
            this.$message({
              type: 'success',
            ...
            this.$message.error('任务状态更新失败1！')
          }
        }, (err) => {
          this.$message.error('任务状态更新失败2！')
          console.log(err)
        })
    },
    remove (index) {
      this.$http.get('/api/todolist/' + this.id + '/' + this.list[index].id)
        .then((res) => {
          if (res.status === 200) {
            this.$message({
              type: 'success',
